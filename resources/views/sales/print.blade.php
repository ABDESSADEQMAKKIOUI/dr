...
<body onload="window.print()">
    @php
        ...
        $taxNumber = \App\Models\Setting::where('key', 'tax_number')->first();
    @endphp
    ...
    <div class="invoice-container">
        <div class="header">
            ...
            <div class="invoice-details">
                ...
                <p>Date: {{ optional($sale->date)->format('M d, Y') }}</p>
                <p>Status: {{ strtoupper($sale->status) }}</p>
            </div>
        </div>
        ...
        <div class="parties">
            <div class="from">
                ...
                @if($companyEmail && $companyEmail->value)
                    Email: {{ $companyEmail->value }}<br>
                @endif
                @if($taxNumber && $taxNumber->value)
                    Tax ID: {{ $taxNumber->value }}
                @endif
                </p>
            </div>
            <div class="to">
                <div class="section-title">To</div>
                <p><strong>{{ $sale->customer->name ?? 'Walk-in Customer' }}</strong><br>
                {{ $sale->customer->address ?? '' }}<br>
                {{ $sale->customer->city ?? '' }} {{ $sale->customer->country ?? '' }}<br>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        {{ $item->product->name }}
                        @if($item->product->code) <span style="color: #999; font-size: 12px;">({{ $item->product->code }})</span> @endif
                    </td>
                    <td>{{ $item->quantity }}</td>
        ...
        <div class="totals">
            ...
            <div class="totals-row">
                <span>Due</span>
                <span>{{ number_format($sale->total_amount - $sale->paid_amount, 2) }} DH</span>
            </div>
        </div>
