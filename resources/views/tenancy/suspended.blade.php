...
<head>
    ...
    <title>{{ __('tenancy.suspended_title') }} — SAFM</title>
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .icon{width:72px;height:72px;border-radius:50%;display:flex;align-items:center;justify-content:center;
            margin:0 auto 24px;background:#fef3c7;color:#d97706}
...
<body>
    <div class="card">
        ...
        <h1>{{ __('tenancy.suspended_title') }}</h1>
        <p>{{ __('tenancy.suspended_body') }}</p>
        @if($tenant && $tenant->contact_email)
            <a class="btn" href="mailto:{{ $tenant->contact_email }}">{{ __('tenancy.contact_support') }}</a>
        @endif
