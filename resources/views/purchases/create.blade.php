@extends('layouts.app')
@section('title', __('app.add_purchase'))
@php
$pageTitle = __('app.add_purchase');
$breadcrumbs = [['label' => __('app.dashboard'), 'url' => route('dashboard')], ['label' => __('app.purchases'), 'url' => route('purchases.index')], ['label' => __('app.create'), 'url' => '']];
@endphp
@section('content')
<div class="card max-w-6xl">
    <div class="card-header"><h3 class="text-lg font-semibold text-gray-800">{{ __('app.add_purchase') }}</h3></div>
    <form method="POST" action="{{ route('purchases.store') }}" data-validate>
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="form-group"><label class="form-label">{{ __('app.supplier') }} *</label><select name="supplier_id" class="form-control" required><option value="">{{ __('app.select_supplier') }}</option>@foreach($suppliers ?? [] as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach</select>@error('supplier_id')<span class="form-error">{{ $message }}</span>@enderror</div>
            ...
            <div class="form-group"><label class="form-label">{{ __('app.date') }} *</label><input type="date" name="purchase_date" value="{{ old('purchase_date', date('Y-m-d')) }}" class="form-control" required></div>
        ...
        <div class="flex justify-end space-x-2 mt-6"><a href="{{ route('purchases.index') }}" class="btn btn-outline">{{ __('app.cancel') }}</a><button type="submit" class="btn btn-primary">{{ __('app.add_purchase') }}</button></div>
    </form>
</div>
@push('scripts')
<script>
