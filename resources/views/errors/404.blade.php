{{--
    ...
    lookups on render -- that would turn a clean 404 into an infinite 500 loop.
--}}
...
<head>
    ...
    <title>{{ $fr ? 'Page introuvable' : 'Page not found' }} — SAFM</title>
    <style>
...
<body>
    <div class="card">
        ...
        <h1>{{ $fr ? 'Page introuvable' : 'Page not found' }}</h1>
        <p>{{ $fr ? "La page que vous recherchez n'existe pas ou a été déplacée." : 'The page you are looking for does not exist or has been moved.' }}</p>
