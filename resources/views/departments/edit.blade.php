@extends('layouts.app')
@section('title', __('app.edit') . ' — ' . $department->name)
@php
$pageTitle = __('app.edit') . ' — ' . $department->name;
$breadcrumbs = [
    ...
    ['label' => $department->name, 'url' => route('departments.show', $department)],
    ...
    <div class="card">
        <div class="card-header">
            <div class="flex items-center gap-3">
                ...
                <h3 class="card-title">{{ __('app.edit') }} — {{ $department->name }}</h3>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('departments.update', $department) }}" data-validate>
            ...
            <div class="flex justify-end gap-3 px-6 pb-6">
                <a href="{{ route('departments.show', $department) }}" class="btn btn-outline">{{ __('app.cancel') }}</a>
