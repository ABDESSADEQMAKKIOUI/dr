@extends('layouts.app')
@section('title', 'Quotations by User')
@php
$pageTitle = 'Quotations by User';
$breadcrumbs = [['label'=>__('app.dashboard'),'url'=>route('dashboard')],['label'=>__('app.reports'),'url'=>'#'],['label'=>'Quotations by User','url'=>'']];
...
<div class="pg-hero">
  ...
  <div><p class="pg-hero-title">Quotations by User</p><p class="pg-hero-sub">Quotation count, value, and conversion rate per staff member.</p></div>
</div>

<div class="rf">
  <div class="rf-head"><span class="rf-title">{{ __('app.filter') }}</span><a href="{{ route('reports.export-csv',['type'=>'quotations-by-user','from'=>$from,'to'=>$to]) }}" class="btn-out" style="padding:.5rem 1rem;font-size:.8125rem">{{ __('app.export_csv') }}</a></div>
...
<div class="mt-wrap">
  <div class="overflow-x-auto">
    <table class="mt">
      <thead><tr><th>{{ __('app.user') }}</th><th>Quotations</th><th>Total Value</th><th>Converted</th></tr></thead>
      <tbody>
      @forelse($rows as $row)
      <tr>
      ...
      @empty
      <tr><td colspan="4" class="text-center py-10 text-gray-400">No data for selected period.</td></tr>
      @endforelse
      </tbody>
      @if($rows->count())
