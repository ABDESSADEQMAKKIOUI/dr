...
$breadcrumbs = [['label'=>__('app.dashboard'),'url'=>route('dashboard')],['label'=>__('app.reports'),'url'=>'#'],['label'=>'Sales by Brand','url'=>'']];
...
<div class="pg-hero">
  ...
  <div><p class="pg-hero-title">Sales by Brand</p><p class="pg-hero-sub">Revenue and quantity sold broken down by product brand.</p></div>
</div>

<div class="rf">
  <div class="rf-head"><span class="rf-title">{{ __('app.filter') }}</span><a href="{{ route('reports.export-csv',['type'=>'sales-by-brand','from'=>$from,'to'=>$to]) }}" class="btn-out" style="padding:.5rem 1rem;font-size:.8125rem">{{ __('app.export_csv') }}</a></div>
...
<div class="mt-wrap">
  <div class="overflow-x-auto">
    <table class="mt">
      <thead><tr><th>Brand</th><th>{{ __('app.qty_sold') }}</th><th>{{ __('app.revenue') }}</th></tr></thead>
      <tbody>
      @forelse($rows as $row)
      <tr>
        <td class="font-medium">{{ $row->brand }}</td>
        <td class="font-semibold">{{ number_format($row->qty) }}</td>
        <td class="font-bold" style="color:{{ $accent }}">{{ number_format($row->revenue,2) }} DH</td>
      </tr>
      @empty
      <tr><td colspan="3" class="text-center py-10 text-gray-400">No data for selected period.</td></tr>
      @endforelse
      </tbody>
      @if($rows->count())
